...
| Style      |  $s=[s_1,s_2,s_3]$ |  $s \sim [\mathcal{U}([-1,1]),\mathcal{U}([-1,1]),\mathcal{U}([-1,1])]$    |   $\tilde{s} \sim {\mathcal{N}_t (s_1,1),\mathcal{N}_t (s_2,1),\mathcal{N}_t (s_3,1)}$ | Stochastic between views |
